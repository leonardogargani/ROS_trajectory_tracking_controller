...
At line 196 of https://docs.ros.org/en/melodic/api/dwa\_local\_planner/html/dwa\_\_planner\_8cpp\_source.html:
\begin{quote}
    // used for visualization only, total\_costs are not really total costs
    bool DWAPlanner::getCellCosts(int cx, int cy, float \&path\_cost, float \&goal\_cost, float \&occ\_cost, float \&total\_cost)

    ...

    total\_cost =
        path\_distance\_bias\_ * path\_cost +
        goal\_distance\_bias\_ * goal\_cost +
        occdist\_scale\_ * occ\_cost;
\end{quote}
