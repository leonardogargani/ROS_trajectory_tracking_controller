...
void bicycle_kin_trajctrl::Prepare(void)
{
    ...
    if (false == Handle.getParam(FullParamName, Kx))
        ROS_ERROR("Node %s: unable to retrieve parameter %s.", ros::this_node::getName().c_str(), FullParamName.c_str());

    FullParamName = ros::this_node::getName()+"/Ky";
    if (false == Handle.getParam(FullParamName, Ky))
        ROS_ERROR("Node %s: unable to retrieve parameter %s.", ros::this_node::getName().c_str(), FullParamName.c_str());

    /* ROS topics */
    vehicleState_subscriber = Handle.subscribe("/car_state", 1, &bicycle_kin_trajctrl::vehicleState_MessageCallback, this);
...
void bicycle_kin_trajctrl::RunPeriodically(float Period)
{
...
void bicycle_kin_trajctrl::Shutdown(void)
{
...
void bicycle_kin_trajctrl::vehicleState_MessageCallback(const std_msgs::Float64MultiArray::ConstPtr& msg)
{
...
void bicycle_kin_trajctrl::PeriodicTask(void)
{
